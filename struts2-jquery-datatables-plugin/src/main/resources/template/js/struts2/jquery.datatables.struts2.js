...
(function($) {
	...
	$.struts2_jquery_datatables = {
		...
		serializeServerSideParamsWithOGNLPath : function(data){
			const params = {draw: data.draw, length: data.length, start: data.start};
			if (data.columns){
		...
		parseDatatablesParams : function($elem, o, params) {
			if (!$elem.data('_s2jdt_init')) {
				...
				if (o.autoFill) {
					self.require("js/plugins/dataTables.autoFill" + self.minSuffix + ".js");
					if (o.theme !== "default") {
						self.require("themes/" + o.theme + "/js/autoFill." + o.theme + self.minSuffix + ".js");
					}
					self.requireCss("themes/" + o.theme + "/css/autoFill." + o.theme + self.minSuffix + ".css");
					if (o.onAutoFillTopics) {
						$elem.on("autoFill.dt", function(event, datatable, cells) {
							const orginal = {};
							orginal.event = event;
							orginal.datatable = datatable;
							orginal.cells = cells;
							orginal.xhr = xhr;
							self.publishTopic($elem, o.onAutoFillTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
					if (o.onBeforeAutoFillTopics) {
						$elem.on("preAutoFill.dt", function(event, datatable, cells) {
							const orginal = {};
							orginal.event = event;
							orginal.datatable = datatable;
							orginal.cells = cells;
				...
				if (o.buttons) {
					self.require("js/plugins/dataTables.buttons" + self.minSuffix + ".js");
					self.require("js/plugins/buttons.html5" + self.minSuffix + ".js");
					self.require("js/plugins/buttons.print" + self.minSuffix + ".js");
					self.require("js/plugins/buttons.colVis" + self.minSuffix + ".js");
					if (o.theme !== "default") {
						self.require("themes/" + o.theme + "/js/buttons." + o.theme + self.minSuffix + ".js");
					}
					self.requireCss("themes/" + o.theme + "/css/buttons." + o.theme + self.minSuffix + ".css");
					if (o.onButtonActionTopics) {
						$elem.on("button-action.dt", function(event, buttonApi, datatable, node, config) {
							const orginal = {};
							orginal.event = event;
							...
							self.publishTopic($elem, o.onButtonActionTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
				}
				if (o.colReorder) {
					self.require("js/plugins/dataTables.colReorder" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/colReorder." + o.theme + self.minSuffix + ".css");
					if (o.onColumnReorderTopics) {
						$elem.on("column-reorder.dt", function(event, settings, details) {
							const orginal = {};
							orginal.event = event;
				...
				if (o.fixedColumns) {
					self.require("js/plugins/dataTables.fixedColumns" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/fixedColumns." + o.theme + self.minSuffix + ".css");
				}
				if (o.fixedHeader) {
					self.require("js/plugins/dataTables.fixedHeader" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/fixedHeader." + o.theme + self.minSuffix + ".css");
				}
				if (o.keys) {
					self.require("js/plugins/dataTables.keyTable" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/keyTable." + o.theme + self.minSuffix + ".css");
					if (o.onKeyBlurTopics) {
						$elem.on("key-blur.dt",
								function(event, datatable, cell) {
									...
									orginal.datatable = datatable;
									orginal.cell = cell;
									self.publishTopic($elem, o.onKeyBlurTopics, orginal);
									self.publishTopic($elem, o.onalw, orginal);
								});
					}
					if (o.onKeyFocusTopics) {
						$elem.on("key-focus.dt", function(event, datatable, cell, originalEvent) {
							const orginal = {};
							orginal.event = event;
							orginal.datatable = datatable;
							orginal.cell = cell;
							orginal.originalEvent = originalEvent;
							self.publishTopic($elem, o.onKeyFocusTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
					if (o.onOtherKeyTopics) {
						$elem.on("key.dt", function(event, datatable, key, cell, originalEvent) {
							const orginal = {};
							orginal.event = event;
							orginal.key = key;
							orginal.datatable = datatable;
							orginal.cell = cell;
							orginal.originalEvent = originalEvent;
							self.publishTopic($elem, o.onOtherKeyTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
				}
				if (o.responsive) {
					self.require("js/plugins/dataTables.responsive" + self.minSuffix + ".js");
					if (o.theme !== "default") {
						self.require("themes/" + o.theme + "/js/responsive." + o.theme + self.minSuffix + ".js");
					}
					self.requireCss("themes/" + o.theme + "/css/responsive." + o.theme + self.minSuffix + ".css");
					if (o.onResponsiveDisplayTopics) {
						$elem.on("responsive-display.dt", function(event, datatable, row, showHide, update) {
							const orginal = {};
							orginal.event = event;
							orginal.datatable = datatable;
							orginal.row = row;
							...
							self.publishTopic($elem, o.onResponsiveDisplayTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
					if (o.onResponsiveResizeTopics) {
						$elem.on("responsive-resize.dt", function(event, datatable, columns) {
							const orginal = {};
							orginal.event = event;
							orginal.datatable = datatable;
							orginal.columns;
							self.publishTopic($elem, o.onResponsiveResizeTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
				}
				if (o.rowReorder) {
					self.require("js/plugins/dataTables.rowReorder" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/rowReorder." + o.theme + self.minSuffix + ".css");
					if (o.onRowReorderTopics) {
						$elem.on("row-reorder.dt",
								function(event, detail, edit) {
									...
									orginal.edit = edit;
									self.publishTopic($elem, o.onRowReorderTopics, orginal);
									self.publishTopic($elem, o.onalw, orginal);
								});
					}
					if (o.onRowReorderedTopics) {
						$elem.on("row-reordered.dt", function(event, detail, edit) {
							const orginal = {};
							orginal.event = event;
							orginal.details = detail;
							orginal.edit = edit;
							self.publishTopic($elem, o.onRowReorderedTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
				}
				if (o.rowGroup) {
					self.require("js/plugins/dataTables.rowGroup" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/rowGroup." + o.theme + self.minSuffix + ".css");
				...
				if (o.scroller) {
					self.require("js/plugins/dataTables.scroller" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/scroller." + o.theme + self.minSuffix + ".css");
				}
				if (o.select) {
					self.require("js/plugins/dataTables.select" + self.minSuffix + ".js");
					self.requireCss("themes/" + o.theme + "/css/select." + o.theme + self.minSuffix + ".css");
					if (o.onDeselectTopics) {
						$elem.on("deselect.dt", function(event, dt, type, indexes) {
							const orginal = {};
							orginal.event = event;
							orginal.dt = dt;
							orginal.type = type;
							orginal.indexes = indexes;
							self.publishTopic($elem, o.onDeselectTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
					if (o.onSelectTopics) {
						$elem.on("select.dt",
								function(event, dt, type, indexes) {
									...
									orginal.type = type;
									orginal.indexes = indexes;
									self.publishTopic($elem, o.onSelectTopics, orginal);
									self.publishTopic($elem, o.onalw, orginal);
								});
					}
					if (o.onSelectItemsTopics) {
						$elem.on("selectItems.dt", function(event, dt, items) {
							...
							orginal.dt = dt;
							orginal.items = items;
							self.publishTopic($elem, o.onSelectItemsTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
					if (o.onSelectStyleTopics) {
						$elem.on("selectStyle.dt", function(event, dt, style) {
							...
							orginal.dt = dt;
							orginal.style = style;
							self.publishTopic($elem, o.onSelectStyleTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
					if (o.onUserSelectTopics) {
						$elem.on("user-select.dt", function(event, dt, type, cell, originalEvent) {
							const orginal = {};
							orginal.event = event;
							orginal.dt = dt;
							orginal.type = type;
							orginal.cell = cell;
							orginal.originalEvent = originalEvent;
							self.publishTopic($elem, o.onUserSelectTopics, orginal);
							self.publishTopic($elem, o.onalw, orginal);
						});
					}
				}
				...
				if (o.ajax) {
					$elem.on("xhr.dt", function(event, settings, json, xhr) {
						if (o.oncom) {
							const orginalEvent = {};
							orginalEvent.event = event;
							orginalEvent.settings = settings;
							orginalEvent.json = json;
							orginalEvent.xhr = xhr;
							self.publishTopic($elem, o.oncom, orginalEvent);
							self.publishTopic($elem, o.onalw, orginalEvent);
						}
						...
						if (json != null) {
							if (o.onsuc) {
								...
								orginalEvent.settings = settings;
								orginalEvent.xhr = xhr;
								self.publishTopic($elem, o.onsuc, orginalEvent);
								self.publishTopic($elem, o.onalw, orginalEvent);
							}
						} else {
							if (o.onerr) {
								...
								orginalEvent.settings = settings;
								orginalEvent.xhr = xhr;
								self.publishTopic($elem, onerr, orginalEvent);
								self.publishTopic($elem, o.onalw, orginalEvent);
				...
				if (o.serverSide){
					...
					if (typeof params.ajax === 'object' && (!params.ajax.contentType || params.ajax.contentType.indexOf('json') === -1)){
						if (params.ajax.data && typeof params.ajax.data =='function'){
							const origFunc = params.ajax.data;
							params.ajax.data = function(d){
								const fixedSSPrequest = self.serializeServerSideParamsWithOGNLPath(d);
								let data = origFunc(fixedSSPrequest);
				...
				if (o.onColumnSizingTopics) {
					$elem.on("column-sizing.dt", function(event, settings) {
						...
						orginal.event = event;
						orginal.settings = settings;
						self.publishTopic($elem, o.onColumnSizingTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
				if (o.onColumnVisibilityTopics) {
					$elem.on("column-visibility.dt", function(event, settings,
						...
						orginal.settings = settings;
						orginal.state = state;
						self.publishTopic($elem, o.onColumnVisibilityTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
				...
				if (o.onProcessingErrorTopics) {
					$elem.on("error.dt", function(event, settings, techNote, message) {
						const orginal = {};
						orginal.event = event;
						orginal.techNote = techNote;
						orginal.settings = settings;
						orginal.message = message;
						self.publishTopic($elem, o.onProcessingErrorTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
				if (o.onInitCompleteTopics) {					
					$elem.on("init.dt", function(event, settings, json) {
						...
						orginal.json = json;
						orginal.settings = settings;
						self.publishTopic($elem, o.onInitCompleteTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
				if (o.onPageLengthChangeTopics) {
					$elem.on("length.dt", function(event, settings, len) {
						...
						orginal.len = len;
						orginal.settings = settings;
						self.publishTopic($elem, o.onPageLengthChangeTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
				...
				if (o.onPageChangeTopics) {
					$elem.on("page.dt",
							function(event, settings, json, xhr) {
								...
								orginal.event = event;
								orginal.settings = settings;
								self.publishTopic($elem, o.onPageChangeTopics, orginal);
								self.publishTopic($elem, o.onalw, orginal);
							});
				}
				...
				if (o.onProcessingTopics) {
					$elem.on("processing.dt",
							function(event, settings, processing) {
								...
								orginal.processing
								orginal.settings = settings;
								self.publishTopic($elem, o.onProcessingTopics, orginal);
								self.publishTopic($elem, o.onalw, orginal);
							});
				}
				if (o.onSearchTopics) {
				...
				if (o.onStateLoadedTopics) {
					$elem.on("stateLoaded.dt", function(event, settings, json) {
						...
						orginal.json = json;
						orginal.settings = settings;
						self.publishTopic($elem, o.onStateLoadedTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
				if (o.onStateLoadingTopics) {
					$elem.on("stateLoadParams.dt", function(event, settings, json) {
						const orginal = {};
						orginal.event = event;
						orginal.json = json;
						orginal.settings = settings;
						self.publishTopic($elem, o.onStateLoadingTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
				if (o.onStateSavingTopics) {
					$elem.on("stateSaveParams.dt", function(event, settings, data) {
						const orginal = {};
						orginal.event = event;
						orginal.data = data;
						orginal.settings = settings;
						self.publishTopic($elem, o.onStateSavingTopics, orginal);
						self.publishTopic($elem, o.onalw, orginal);
					});
				}
			}
		...
		datatables : function($elem, o) {
			...
			if (self.datatablesLocal !== 'en') {
				params.language = {
					url : path + "i18n/datatables.locale-" + self.datatablesLocal + ".json"
				};
			}
			
			self.requireCss("themes/" + o.theme + "/css/dataTables." + o.theme + self.minSuffix + ".css");
			if (o.theme !== "default") {
				self.require("themes/" + o.theme + "/js/dataTables." + o.theme + self.minSuffix + ".js");
			}
			$.extend(params, o);
