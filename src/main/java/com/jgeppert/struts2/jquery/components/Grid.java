...
public class Grid extends AbstractRemoteBean
{
	...
	public void evaluateExtraParams() {
		...
		if (subGrid != null)
		{
			addParameter("subGrid", findValue(this.subGrid, Boolean.class));
			...
			if ((this.id == null || this.id.length() == 0))
			{
				...
				this.id = "subgrid_" + String.valueOf(nextInt);
	...
	@StrutsTagAttribute(description = "If set to true this enables using a subgrid. If the subGrid is enabled a additional column at left side is added to the basic grid. This column contains a 'plus' image which indicate that the user can click on it to expand the row. By default all rows are collapsed.", defaultValue = "false", type = "Boolean")
